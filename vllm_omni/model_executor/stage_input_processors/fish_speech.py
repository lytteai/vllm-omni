...
def _cpu_long_vector(tensor: torch.Tensor) -> torch.Tensor:
    """One host copy of a codebook frame. Avoid extra GPU `.item()` syncs."""
    frame = tensor.detach().reshape(-1)
    if frame.device.type != "cpu" or frame.dtype != torch.long:
...
def _extract_last_frame(multimodal_output: dict[str, Any]) -> torch.Tensor | None:
    ...
    if audio_codes.ndim == 2:
        frame = _cpu_long_vector(audio_codes[-1])
        valid = multimodal_output.get("audio_code_valid")
        if isinstance(valid, torch.Tensor) and valid.numel() > 0:
            is_valid = bool(_cpu_long_vector(valid.reshape(-1)[-1]).item())
        elif valid is not None:
            is_valid = bool(valid)
        else:
            is_valid = bool(frame.any())
        if frame.numel() == 0 or not is_valid:
            return None
        return frame
    if audio_codes.ndim == 1:
        return _cpu_long_vector(audio_codes)
    raise ValueError(f"Invalid audio_codes shape for Fish Speech async_chunk: {tuple(audio_codes.shape)}")


def _read_request_initial_chunk_frames(request: Any, default: int) -> int:
    """Prefer the per-request override; accept engine entries or a plain dict."""
    info = getattr(request, "additional_information", None)
    if info is None:
        return default

    entries = getattr(info, "entries", None)
    if isinstance(entries, dict) and "initial_codec_chunk_frames" in entries:
        ...
        if list_data is not None and len(list_data) == 1:
            return int(list_data[0])
        scalar = getattr(entry, "scalar_data", None)
        if scalar is not None:
            return int(scalar)

    if isinstance(info, dict) and "initial_codec_chunk_frames" in info:
        value = info["initial_codec_chunk_frames"]
...
def slow_ar_to_dac_decoder_async_chunk(
    ...
    initial_chunk_size = _read_request_initial_chunk_frames(request, configured_initial_chunk_size)
    request_overrides_initial = initial_chunk_size != configured_initial_chunk_size

    if not hasattr(transfer_manager, "_ramp_parsed"):
        transfer_manager._ramp_parsed = parse_chunk_ramp(cfg, steady=chunk_size)
    # A per-request IC override is an A/B of first-audio latency; it wins over ramp.
    ramp = None if request_overrides_initial else transfer_manager._ramp_parsed
